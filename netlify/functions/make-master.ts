...
import type { CopyRecord, MakeMasterRequest, NarrationVersion } from '../../src/types/index.js'

/**
 * Torna uma versão específica o master (equivalente a aprovar direto do
 * histórico). A versão precisa ter áudio válido.
 */
export const handler: Handler = async (event) => {
  try {
    ...
    const payload = JSON.parse(event.body || '{}') as MakeMasterRequest
    ...
    if (version.status === 'ERROR' || !version.audio_key) {
      return badRequest('Não é possível tornar master uma versão sem áudio válido.')
    }
